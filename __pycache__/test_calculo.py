...
def test_calculate_total(price, discount_rate, tax_rate):
    excepted_discount = price * discount_rate
    except_tax = (price - excepted_discount) * tax_rate
    except_total = price - excepted_discount + except_tax

    assert calculate_total(price, discount_rate, tax_rate) == round(except_total,2)